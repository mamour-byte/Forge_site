...
const DEFAULT_WORDS = ["FORGE", "VOS SOLUTIONS", "DIGITAL", "SUR MESURE"]
...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
  ...
  const animate = () => {
    ...
    const ctx = canvas.getContext("2d")!
    const particles = particlesRef.current
    const backgroundColor = getComputedStyle(document.documentElement).getPropertyValue("--background").trim() || "#f4f4f4"
  ...
  useEffect(() => {
    ...
    // Use parent width or fallback to window width
    const parentWidth = canvas.parentElement?.clientWidth || window.innerWidth || 800
